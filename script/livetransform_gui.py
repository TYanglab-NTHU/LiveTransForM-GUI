...
class Single_mutation(tk.Frame,Genlig):
    def __init__(self, master=None):
        ...
        self.title.pack()
        self.path_entry = tk.Entry(self.path_frame, font="Helvetica 14", width=30)
        self.get_path = tk.Button(self.path_frame, text="確認", command=self.get_result_path)
    ...
    def Single_mutation(self, ligand_choose=None, idx=None, SS=False, scs_limit=None, step_size=0.01, step_size_limit=0, max_step_size_limit=100):
        ...
        #     img.destroy()
        # self.output_images = []
        for label, img in self.output_labels:
            label.destroy()
        self.output_labels = []
        result_list = []
        ...
        if selected_ligand:
            ...
            if interpolation:
                ...
                for i in mutation_list:
                    ...
                    result_list.append([idx, smiles])
                    self.output_labels.append((dig_label, ligand_image_tk))
                    img_data.save(os.path.join(single_mutation_dir, f'{smiles}.png'))

                with open(os.path.join(single_mutation_dir, 'single_muta_output.csv'), 'w') as f:
                    f.write("idx, smiles \n")
                    for idx, smiles in result_list:
                        f.write("{}, {} \n".format(idx, smiles))    
            ...
            if metal_atom_count != 1:
                ...
                if len(spin_state_prediction) != 1:
                    ...
                    with open('{}/seeded_gen_output.csv'.format(filepath), 'w') as f:
                        ...
                        for idx, (original_lig, mutated_lig) in enumerate(comparsion_ligs):
                            f.write("{}, {}, {} \n".format(idx, original_lig, mutated_lig))
